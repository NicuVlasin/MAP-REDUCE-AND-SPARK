...
def my_reduce(input_stream, total_petitions, output_stream):
    # It should look like this: en (total_visits, percentage)
    ...
    for line in input_stream:
        ...
        if(lang_project not in language_project_dict):
            language_project_dict[lang_project] = visits
        else:
            language_project_dict[lang_project] = language_project_dict.get(lang_project) + visits

    for key,value in language_project_dict.items():
        line_to_print = key + "(" + str(value), str((value/total_petitions)*100) + ")"
        print(line_to_print)
